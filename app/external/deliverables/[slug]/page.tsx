...
import { getDeliverableContent } from '@/lib/deliverables'
...
// Need proper typing for Next.js 15
interface Props {
...
export default async function DeliverablePage({ params }: Props) {
  ...
  const meta = DELIVERABLES.find(d => d.slug === slug)
  if (!meta) notFound()

  const content = getDeliverableContent(slug)
  const currentIndex = DELIVERABLES.findIndex(d => d.slug === slug)
  const prev = currentIndex > 0 ? DELIVERABLES[currentIndex - 1] : null
  const next = currentIndex < DELIVERABLES.length - 1 ? DELIVERABLES[currentIndex + 1] : null
      ...
      <div className="pt-16 min-h-screen flex bg-white">
        ...
        <aside className="hidden lg:flex flex-col w-60 flex-shrink-0 border-r border-gray-100 py-8 sticky top-16 h-[calc(100vh-4rem)] overflow-y-auto">
          <div className="px-4 mb-4">
            <p className="text-[10px] font-bold text-[#80848f] uppercase tracking-widest">External Analysis</p>
          </div>
          ...
          <div className="mt-auto px-4 pt-6 border-t border-gray-100 mx-2">
            <Link href="/external/presentation" className="flex items-center gap-2 text-xs text-[#1B3A6B] hover:text-[#122847] transition-colors font-medium">
              ← View Presentation
            </Link>
          </div>
        </aside>
        ...
        <main className="flex-1 min-w-0">
          <div className="max-w-3xl mx-auto px-8 py-10">
            ...
            <div className="flex items-center gap-2 text-sm text-[#80848f] mb-6">
              ...
              <span>/</span>
              <Link href="/external/deliverables" className="hover:text-[#1B3A6B] transition-colors">Deliverables</Link>
              <span>/</span>
            ...
            <div className="flex justify-between items-center mt-16 pt-8 border-t border-gray-100">
              {prev ? (
                <Link
                  href={`/external/deliverables/${prev.slug}`}
                  className="flex items-center gap-2 text-sm text-[#80848f] hover:text-[#1B3A6B] transition-colors group"
                >
              ...
              {next ? (
                <Link
                  href={`/external/deliverables/${next.slug}`}
                  className="flex items-center gap-2 text-sm text-[#80848f] hover:text-[#1B3A6B] transition-colors group text-right"
                >
                  <div>
